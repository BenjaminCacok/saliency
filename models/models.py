...
class GSGNet(nn.Module):
    def __init__(self, backbone_dim=[2208, 2112, 768, 384, 96], embed_dim=[192, 192, 192, 192],
                 backbone_type='densenet161'):
        super().__init__()
        # backbone
...
class GSGNet_T(nn.Module):
    def __init__(self, backbone_dim=[2208, 2112, 768, 384, 96], embed_dim=[192, 192, 192, 192], setting=0.25,
                 backbone_type='densenet161'):
        super().__init__()
        # backbone
        self.backbone = timm.create_model(backbone_type, pretrained=False, features_only=True)
        # top layer
        self.toplayer = nn.Conv2d(backbone_dim[0], embed_dim[0], 1, 1, 0)
        # lateral layers
        self.latlayer1 = nn.Conv2d(backbone_dim[1], embed_dim[1], 1, 1, 0)
        ...
        self.lgfb_1 = LocalGlobalFusionBlock(dim_q=embed_dim[1] // 2, dim_kv=embed_dim[1], factor=2, keep_ratio=setting,
                                             n_heads=4)
        self.lgfb_2 = LocalGlobalFusionBlock(dim_q=embed_dim[2] // 2, dim_kv=embed_dim[2], factor=4, keep_ratio=setting,
                                             n_heads=4)
        self.lgfb_3 = LocalGlobalFusionBlockL(dim_q=embed_dim[3] // 2, dim_kv=embed_dim[3], factor=2, scale_factor=0.25,
                                              keep_ratio=setting, n_heads=4)
